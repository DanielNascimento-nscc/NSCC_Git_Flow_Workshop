...
public class MenuCreator : MonoBehaviour
{
    ...
    public TextField AddTextField(string Label, EventCallback<ChangeEvent<string>> ChangedCallback, string defaultText = "", int maxLength = 25, bool isPassword = false)
    {
    ...
    public const string ThreeButtonHolderClass = "ThreeButtonHolder";
    protected void AddThreeButton(MenuButton menuButton1, MenuButton menuButton2, MenuButton menuButton3)
    {
        VisualElement ThreeButtonHolder = new VisualElement();
        m_MenuPanel.Add(ThreeButtonHolder);
        ThreeButtonHolder.AddToClassList(ThreeButtonHolderClass);
        createdVisualElements.Add(ThreeButtonHolder);
        // Button 1
        var newButton1 = new Button(menuButton1.ButtonAction);
        newButton1.text = menuButton1.ButtonText;
        ThreeButtonHolder.Add(newButton1);
        createdVisualElements.Add(newButton1);
        menuButton1.SetButtonColor(newButton1);
        // Button 2
        var newButton2 = new Button(menuButton2.ButtonAction);
        newButton2.text = menuButton2.ButtonText;
        ThreeButtonHolder.Add(newButton2);
        createdVisualElements.Add(newButton2);
        menuButton2.SetButtonColor(newButton2);
        // Button 3
        var newButton3 = new Button(menuButton3.ButtonAction);
        newButton3.text = menuButton3.ButtonText;
        ThreeButtonHolder.Add(newButton3);
        createdVisualElements.Add(newButton3);
        menuButton3.SetButtonColor(newButton3);
    }
...
//        Debug.Log("Called hide menu");
    ...
    public void CreateMenuWithThreeButtonWhenPossible(params MenuButton[] menuButtons)
    {
        m_MenuHolder.SetDisplayBasedOnBool(true);
        m_MenuHolder.style.opacity = 0;
        DOTween.To(()=> m_MenuHolder.style.opacity.value, x=> m_MenuHolder.style.opacity = x, 1, 0.33f).SetEase(Ease.InOutQuad);
        ...
        for (int i = 0; i < menuButtons.Length; i++)
        {
            ...
            if (i + 2 < menuButtons.Length)
            {
                AddThreeButton(menuButtons[i], menuButtons[i+1], menuButtons[i+2]);
                skipCount = 2;
            }
            else if (i + 1 < menuButtons.Length)
            {
                AddTwoButton(menuButtons[i], menuButtons[i+1]);
                skipCount = 1;
            }
...
public static class VisualElementExtensionMethods
{
    ...
    public static TweenerCore<float,float, FloatOptions> DoFade(this VisualElement vs, float endValue, float duration = 0.33f, Ease easeType = Ease.InOutCubic) {
        if (endValue > 0.1f && vs.style.display != DisplayStyle.Flex) vs.SetDisplayBasedOnBool(true);
        return DOTween.To(() => vs.style.opacity.value, x => vs.style.opacity = x, endValue, duration).SetEase(easeType).OnComplete((() => { vs.SetDisplayBasedOnBool(endValue > 0.1f); }));
    }
    ...
    public static void SetDisplayBasedOnBool(this VisualElement vs, bool b, bool updatePickingMode = true)
    {
        vs.style.display =
            b ? new StyleEnum<DisplayStyle>(DisplayStyle.Flex) : new StyleEnum<DisplayStyle>(DisplayStyle.None);
        if (updatePickingMode)
            vs.pickingMode = b ? PickingMode.Ignore : PickingMode.Position;
    }
